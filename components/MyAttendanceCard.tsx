    ...
    return (
        <div className="bg-white rounded-lg shadow-md p-6 border-t-4 border-brand-primary">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold text-gray-800">Accountability Profile</h3>
                <div className="flex items-center gap-2">
                    {streak >= 2 && (
                         <span className="inline-flex items-center gap-1 bg-orange-50 text-orange-600 text-[10px] font-black uppercase px-2 py-1 rounded-full border border-orange-200 animate-pulse">
                            🔥 {streak} Service Streak
                        </span>
                    )}
                    <span className={`text-[10px] font-black px-2 py-1 rounded bg-gray-100 uppercase tracking-widest ${textColor} border`}>
                        {reliability >= 95 ? 'ELITE' : reliability >= 85 ? 'RELIABLE' : 'GROWING'}
                    </span>
                </div>
            </div>
            ...
            {stats.totalAssignments > 0 ? (
                <>
                    ...
                    <p className="text-[10px] text-gray-400 mt-4 text-center uppercase tracking-tighter">Reliability is weighted: Absences affect score significantly. Late check-ins count as 50% credit.</p>
                </>
            ) : (
                <div className="mt-4 text-center py-10 bg-gray-50 rounded-lg border-2 border-dashed">
                    <p className="text-sm text-gray-500 px-8">Your accountability profile will begin once you complete your first assigned service.</p>
                </div>
            )}
        </div>
    );
